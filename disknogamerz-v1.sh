...
# Color Codes for Professional GUI Styling
C_CYAN='\033[1;36m'
C_BLUE='\033[1;34m'
C_GREEN='\033[1;32m'
C_YELLOW='\033[1;33m'
...
get_ram_usage() {
    free | awk '/Mem:/ {printf "%.0f%% (Used: %dMB / Total: %dMB)\n", ($3/$2)*100, $3/1024, $2/1024}'
}
...
show_dashboard() {
    ...
    echo -e "${C_CYAN}│${C_RESET} ${C_BLUE}[ Host: ${HOST_ID} ]${C_RESET}  ${C_PURPLE}[ Uptime: ${UPTIME_STR} ]${C_RESET}  ${C_GREEN}[ RAM: ${RAM_STAT} ]${C_RESET} ${C_CYAN}│${C_RESET}"
    echo -e "${C_CYAN}└────────────────────────────────────────────────────────────────────────┘${C_RESET}"
    echo -e ""
    echo -e "${C_CYAN}  ██████╗ ██╗███████╗██╗  ██╗███╗   ██╗ ██████╗  █████╗ ███╗   ███╗ █████╗ ███████╗███████╗███████╗${C_RESET}"
    echo -e "${C_CYAN}  ██╔══██╗██║██╔════╝██║ ██╔╝████╗  ██║██╔════╝ ██╔══██╗████╗ ████║██╔══██╗╚══███╔╝██╔════╝╚══███╔╝${C_RESET}"
    ...
    echo -e "${C_CYAN}  ██████╔╝██║███████║██║  ██╗██║ ╚████║╚██████╔╝██║  ██║██║ ╚═╝ ██║██║  ██║███████╗███████╗███████╗${C_RESET}"
    echo -e "${C_CYAN}╚═════════╩═╩═══════╩═╝  ╚═╝╚═╝  ╚═══╝ ╚═════╝ ╚═╝  ╚═╝╚═╝     ╚═╝╚═╝  ╚═╝╚═══════╚═══════╚═══════╝${C_RESET}"
    echo -e "${C_WHITE}                  DISKNOGAMERZ V1 — OBSIDIAN EDITION                  ${C_RESET}"
    echo -e ""
    echo -e "${C_YELLOW}⚙ SYSTEM STATUS${C_RESET}"
    echo -e "   CPU Usage : ${CPU_STAT}"
    echo -e "   RAM Usage : ${RAM_STAT}"
    echo -e "   Network   : ${NET_STAT}"
    echo -e ""
    echo -e "${C_CYAN}📦 DEPLOYMENT & SERVICES${C_RESET}"
    echo -e "   [1] VPS Virtualization      [5] Pterodactyl Themes & Blueprint"
    echo -e "   [2] Panel Manager           [6] System Operations"
    ...
    echo -e ""
    echo -e "${C_GREEN}➜${C_RESET} Enter Option ${C_WHITE}(0-9)${C_RESET}: "
}

# Feature Menus Implementation
run_vps_menu() {
    while true; do
        clear
        echo -e "${C_CYAN}=== [1] VPS VIRTUALIZATION MENU ===${C_RESET}"
...
run_panel_menu() {
    while true; do
        clear
        echo -e "${C_CYAN}=== [2] PTERODACTYL & PANEL MANAGER ===${C_RESET}"
        echo -e " 1) Install Pterodactyl Panel"
        echo -e " 2) Install NovaStudio / JexPanel / Reiviactyl"
        echo -e " 3) Panel User Management & Add IP"
        echo -e " 0) Back to Main Menu"
...
while true; do
    ...
    case $choice in
        ...
        0) echo -e "${C_RED}Exiting Disknogamerz V1. Goodbye!${C_RESET}"; exit 0 ;;
